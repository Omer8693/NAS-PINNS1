"""
NAS-PINN baseline for Poisson equation on L-shaped domain
...
# Domain definition: L-shaped region
# Example: union of [0,2]x[0,1] and [0,1]x[1,2] inside [-1,1]x[-1,1]
def is_in_lshape(x, y):
    return ((x >= 0) & (x <= 2) & (y >= 0) & (y <= 1)) | ((x >= 0) & (x <= 1) & (y >= 1) & (y <= 2))
...
# Sampling functions
# Rejection sampling for collocation points inside L-shaped domain
def sample_collocation_points(n):
    x = []
    y = []
    while len(x) < n:
        x_try = np.random.uniform(0, 2, n)
        y_try = np.random.uniform(0, 2, n)
        mask = is_in_lshape(x_try, y_try)
        x.extend(x_try[mask])
        y.extend(y_try[mask])
    return np.array(x[:n]), np.array(y[:n])

# Boundary points: sample along L edges
def sample_boundary_points(n):
    # Sample more points at corners
    n_edges = 6
    n_per_edge = n // n_edges
    edges = [
        ((0, 0), (2, 0)),
        ((2, 0), (2, 1)),
        ((2, 1), (1, 1)),
    ...
    for (start, end) in edges:
        xs = np.linspace(start[0], end[0], n_per_edge)
        ys = np.linspace(start[1], end[1], n_per_edge)
        x.extend(xs)
        y.extend(ys)
...
def train_naspinn(layers=4, base_neurons=110, mask_levels=(30,50,70,90,110),
                  collocation_n=4000, boundary_n=800, outer_n=4000,
...
if __name__ == "__main__":
    ...
    save_dir = "results/poisson/lshape/"
    ...
    for repeat in range(5):
        ...
        grid_n = 200
        x_grid = np.linspace(0, 2, grid_n)
        y_grid = np.linspace(0, 2, grid_n)
        X, Y = np.meshgrid(x_grid, y_grid)
        mask = is_in_lshape(X, Y)
        ...
        plt.imshow(u_pred, extent=[0,2,0,2], origin="lower", cmap="coolwarm")
        plt.colorbar()
        plt.title("Predicted Solution")
        plt.savefig(os.path.join(save_dir, f"solution_repeat{repeat+1}.png"))
        plt.close()
        plt.figure(figsize=(6,5))
        plt.imshow(error, extent=[0,2,0,2], origin="lower", cmap="magma")
        plt.colorbar()
