# At the end of main or after training, add:
import gc
gc.collect()
import torch
...
RADIUS = 1.0
# Further reduced for memory safety
N_COL = 1000
N_BC = 200
TEST_GRID_SIZE = 300
...
def sample_points(n_col=N_COL, n_bc=N_BC, device=None, radius=RADIUS):
    ...
    theta = 2 * np.pi * torch.rand(n_col, device=device)

    x_col = (r * torch.cos(theta)).unsqueeze(1)
    y_col = (r * torch.sin(theta)).unsqueeze(1)

    theta_bc = torch.linspace(0, 2*np.pi, n_bc, device=device)
