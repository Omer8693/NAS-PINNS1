...
def plot_runtime_vs_objective(df: pd.DataFrame, out_png: Path) -> None:
    if df.empty:
        return
    plot_df = df.copy()
    plot_df["run_time_seconds"] = pd.to_numeric(plot_df["run_time_seconds"], errors="coerce")
    plot_df["best_objective"] = pd.to_numeric(plot_df["best_objective"], errors="coerce")
    plot_df = plot_df.dropna(subset=["group", "run_time_seconds", "best_objective"])
    if plot_df.empty:
        return

    groups = sorted(plot_df["group"].astype(str).unique().tolist())
    n_groups = len(groups)
    ncols = 2 if n_groups > 1 else 1
    nrows = int(np.ceil(n_groups / ncols))
    fig, axes = plt.subplots(nrows, ncols, figsize=(7.4 * ncols, 4.8 * nrows), sharey=True)
    ...
    for idx, group in enumerate(groups):
        ...
        for _, row in gdf.iterrows():
            stage = str(row.get("best_stage", "")).strip().lower()
            method = str(row.get("method", "")).strip().lower()
            color = stage_colors.get(stage, "#444444")
            marker = method_markers.get(method, "o")
            ax.scatter(
                float(row["run_time_seconds"]),
            ...
            ax.annotate(
                str(row["scenario"]),
                (float(row["run_time_seconds"]), float(row["best_objective"])),
        ...
        ax.set_yscale("log")
        ax.set_title(f"{group} ({len(gdf)} runs)")
        ax.set_xlabel("run_time_seconds")
        if idx % ncols == 0:
            ax.set_ylabel("best_objective (log scale, lower is better)")
        ax.grid(alpha=0.25)

    for idx in range(n_groups, len(axes_arr)):
        axes_arr[idx].axis("off")

    fig.suptitle("Runtime vs Objective (Panel by Group)")
...
def load_quality_input(results_root: Path) -> pd.DataFrame:
    # Primary source
    primary = read_csv(results_root / "best_architectures_summary_ranked.csv")
    if not primary.empty:
        return primary

    # Fallback: previous report table
    fallback_report = read_csv(results_root / "report_pack" / "tables" / "quality_summary.csv")
    if not fallback_report.empty:
        return fallback_report

    # Fallback: archived copy (prefer newest _archive_* folder)
    archive_dirs = sorted([p for p in results_root.glob("_archive_*") if p.is_dir()], reverse=True)
    for archive_root in archive_dirs:
        archived = read_csv(archive_root / "best_architectures_summary_ranked.csv")
...
def main():
    ...
    quality_in = load_quality_input(results_root)
