...
class NSGA3(GeneticAlgorithm):
        ...
        **kwargs,
        ...
        if self.ref_dirs is not None:
            ...
            if pop_size < len(self.ref_dirs):
                print(
                    f"WARNING: pop_size={pop_size} is less than number of reference directions={len(self.ref_dirs)}."
                )
    ...
    def _setup(self, problem, **kwargs):
        if self.ref_dirs is not None and self.ref_dirs.shape[1] != problem.n_obj:
            raise Exception(
                "Dimensionality of reference points must be equal to number of objectives: "
...
class BurgersNSGA3Problem(ElementwiseProblem):
...
class NSGAIICallback:
    def __init__(self, n_gen):
        self.n_gen = n_gen
        self.pbar = tqdm(total=n_gen, desc="NSGA-III generation", unit="gen")
...
def build_ref_dirs_2d(n_partitions):
    n_partitions = max(int(n_partitions), 2)
    a = np.linspace(0.0, 1.0, n_partitions + 1)
...
def run_nsga3_search(
    ...
    print("\n" + "=" * 90)
    print("NSGA-III NAS-PINN Search")
    print("=" * 90)
    print(f"  Viscosity: nu={nu_coef:.6f} | pop={pop} | gen={ngen} | eval_epochs={eval_epochs}")
    print("  Objective: min (relative L2, params)")
    print("=" * 90 + "\n")

    ref_dirs = build_ref_dirs_2d(ref_partitions)

    problem = BurgersNSGA3Problem(nu_coef, fixed_points, eval_epochs, max_hidden_layers, test_nt, test_nx)
    algorithm = NSGA3(
        ref_dirs=ref_dirs,
        pop_size=pop,
        sampling=FloatRandomSampling(),
...
def parse_args():
    parser = argparse.ArgumentParser(description="NAS-PINN Burgers training and plotting (NSGA-III)")
    parser.add_argument("--checkpoint", type=str, default="checkpoint_last_nsga3.pth", help="checkpoint path")
    ...
    parser.add_argument("--save-dir", type=str, default="results/burgers/nsga3", help="directory to save plot images")
    parser.add_argument("--seed", type=int, default=42, help="base random seed")

    parser.add_argument("--skip-nsga", action="store_true", help="skip NSGA-III and use manual/default architecture")
    parser.add_argument("--nsga-pop", type=int, default=10, help="NSGA-III population size")
    parser.add_argument("--nsga-gen", type=int, default=5, help="NSGA-III generations")
    parser.add_argument("--nsga3-partitions", type=int, default=12, help="reference-direction partitions for NSGA-III")
...
def run_single(args):
    ...
    if args.plot_only:
        ...
        if args.skip_nsga:
            ...
            search_result = run_nsga3_search(
                nu_coef=args.nu,
                ...
                seed=args.seed,
                ref_partitions=args.nsga3_partitions,
            )
...
def run_multi_nu(args):
    ...
    for idx, nu_val in enumerate(nu_values):
        ...
        args_local.checkpoint = "checkpoint_last_nsga3.pth"
    ...
    plt.title("Burgers NSGA-III: Viscosity Comparison")
