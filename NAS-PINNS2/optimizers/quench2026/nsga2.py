...
class Quench2026NSGA2Problem(ElementwiseProblem):
...
def run_search(args, method_dir: Path, log_path: Path):
    problem = Quench2026NSGA2Problem(args, method_dir, log_path)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Quench2026 NAS-PINN with NSGA-II architecture search")
    add_shared_args(parser, default_save_dir="results/quench2026/nsga2")
...
def run(args):
    return run_method("nsga2", args, run_search)
