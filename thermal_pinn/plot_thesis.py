...
from thermal_pinn.plot_results import (
...
DOMAINS_2D_LIST = ["rectangle", "circle", "annulus", "lshape"]
...
def _collect_all_k_data(registry: list[dict], suffix: str = "") -> list[dict]:
    """
    ...
    for domain_name, dim in all_combinations:
        for arch in _ARCH_ORDER:
            for k in range(1, 6):
                ...
                mean_l2         = float(np.mean([w["l2_rel"]    for w in wins]))
                ...
                fem_total       = 21
                steps_saved     = fem_total - n_windows
                fem_step_time   = 60.0
                fem_total_time  = fem_total * fem_step_time
                speedup         = fem_total_time / max(total_runtime, 1.0)
...
def _collect_best_k_data(registry: list[dict], suffix: str = "") -> list[dict]:
    """
...
def fig_th2a_matrix_allk(registry: list[dict], out_dir: Path, suffix: str = ""):
    ...
    rows = _collect_all_k_data(registry, suffix=suffix)
    if not rows:
    ...
    out_path = out_dir / "fig_th2a_l2_matrix.png"
    fig.savefig(out_path, dpi=_FIG_DPI, bbox_inches="tight", facecolor=_WHITE)
    plt.close(fig)
    print(f"    → {out_path.name}")
...
def fig_th2b_table_bestk(registry: list[dict], out_dir: Path, suffix: str = ""):
    ...
    col_headers = [
        ...
        "Windows", "Steps saved", "Speedup", "Notes",
    ]
    n_cols = len(col_headers)
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "fig_th2b_best_k_table.png"
    fig.savefig(out_path, dpi=_FIG_DPI, bbox_inches="tight", facecolor=_WHITE)
    plt.close(fig)
    print(f"    → {out_path.name}")
...
if __name__ == "__main__":
    ...
    if run_all or args.fig == 2:
        fig_th2a_matrix_allk(reg, summary_dir, suffix=suffix)
        fig_th2b_table_bestk(reg, summary_dir, suffix=suffix)
