...
class NSGA3Problem(ElementwiseProblem):
...
def run_architecture_search(domain_mod, args):
    print(f"Starting NSGA-III architecture search: domain={args.domain}")
    problem = NSGA3Problem(domain_mod, proxy_epochs=args.proxy_epochs)
...
def run_single(args, domain_name, save_dir):
    ...
    plot_loss_history(
        loss_hist,
        save_path=os.path.join(save_dir, "poisson_nsga3_loss_curve.png"),
        title=f"Poisson NSGA-III Loss ({domain_name})",
    )
    eval_model = FixedMaskModel(model, best_masks).to(device)
    rel_l2 = evaluate_and_plot(eval_model, domain_mod, save_dir=save_dir, method_label="NSGA-III")

    run_time = time.perf_counter() - start
    save_run_metrics(save_dir, "nsga3", domain_name, args.seed, rel_l2, run_time)
...
def run_multi_domain(args):
    ...
    plt.title("Poisson NSGA-III: Domain Comparison")
...
def parse_args():
    ...
    parser.add_argument("--n-gen", type=int, default=N_GEN, help="NSGA-III generation count")
    parser.add_argument("--ref-partitions", type=int, default=12, help="reference direction partitions")
