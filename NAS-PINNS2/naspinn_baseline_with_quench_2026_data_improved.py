...
def data_loss(model, reference_data):
    ...
    temp_target = reference_data["target_temp"]
    # Normalized temperature regression keeps paper-data term numerically competitive.
    temp_mean = torch.mean(temp_target)
    temp_std = torch.std(temp_target, unbiased=False).clamp_min(1e-6)
    l_temp = torch.mean((((T_pred - temp_mean) / temp_std) - ((temp_target - temp_mean) / temp_std)) ** 2)
    ...
    disp_target = reference_data["target_uy"]
    disp_mean = torch.mean(disp_target)
    disp_std = torch.std(disp_target, unbiased=False).clamp_min(1e-9)
    l_disp = torch.mean((((uy_pred - disp_mean) / disp_std) - ((disp_target - disp_mean) / disp_std)) ** 2)

    return l_temp + l_disp, l_temp, l_disp


def compute_reference_mae(model, reference_data):
    with torch.no_grad():
        inp_temp = torch.cat(
            [reference_data["x_temp"], reference_data["y_temp"], reference_data["t_temp"]],
            dim=1,
        ...
        t_pred = model(inp_temp)[:, 0:1]
        temp_mae = torch.mean(torch.abs(t_pred - reference_data["target_temp"]))

        # Fair displacement metric: paper reference only at x=0, final time layer points.
        inp_disp = torch.cat(
            [reference_data["x_disp"], reference_data["y_disp"], reference_data["t_disp"]],
            dim=1,
        )
        uy_pred = model(inp_disp)[:, 2:3]
        disp_mae = torch.mean(torch.abs(uy_pred - reference_data["target_uy"]))

    return float(temp_mae.item()), float(disp_mae.item())


def compute_stage_score(objective, temp_mae, disp_mae, reference_data, args):
    temp_span = float(
        (reference_data["target_temp"].max() - reference_data["target_temp"].min()).detach().cpu().item()
    )
    disp_scale = float(torch.mean(torch.abs(reference_data["target_uy"])).detach().cpu().item())
    temp_norm = temp_mae / max(temp_span, 1e-6)
    disp_norm = disp_mae / max(disp_scale, 1e-9)
    score = (
        float(objective)
        + float(args.stage_score_w_temp) * float(temp_norm)
        + float(args.stage_score_w_disp) * float(disp_norm)
    )
    return score, temp_norm, disp_norm
...
def weighted_total_loss_with_weights(loss_dict, w_physics, w_ic, w_bc, w_data):
    return (
        float(w_physics) * loss_dict["physics"]
        + float(w_ic) * loss_dict["ic"]
        + float(w_bc) * loss_dict["bc"]
        + float(w_data) * loss_dict["data"]
    )


def weighted_total_loss(loss_dict, args):
    return weighted_total_loss_with_weights(
        loss_dict,
...
def get_curriculum_weights(epoch, args):
    if epoch < int(args.phase1_epochs):
        return {
            "name": "phase1",
            "w_physics": float(args.phase1_w_physics),
            "w_ic": float(args.phase1_w_ic),
            "w_bc": float(args.phase1_w_bc),
            "w_data": float(args.phase1_w_data),
        }
    return {
        "name": "phase2",
...
def train_baseline(args):
    ...
    args.phase1_epochs = max(0, min(int(args.phase1_epochs), int(args.epochs)))
    ...
    print(
        "Loss weights (phase1) | "
        f"physics={args.phase1_w_physics:.3e}, "
        f"ic={args.phase1_w_ic:.3e}, "
        f"bc={args.phase1_w_bc:.3e}, "
        f"data={args.phase1_w_data:.3e} | "
    ...
    if load_checkpoint_if_exists(model, adam_ckpt_path):
        ...
        adam_temp_mae, adam_disp_mae = compute_reference_mae(model, reference_data)
        adam_score, adam_temp_norm, adam_disp_norm = compute_stage_score(
            adam_obj, adam_temp_mae, adam_disp_mae, reference_data, args
        )
    else:
        ...
        for epoch in range(args.epochs):
            ...
            w = get_curriculum_weights(epoch, args)
            total = weighted_total_loss_with_weights(
                losses,
                w_physics=w["w_physics"],
                w_ic=w["w_ic"],
                w_bc=w["w_bc"],
                w_data=w["w_data"],
            )
    ...
    stage_rows = [
        {
            "stage": "adam",
            "objective": float(adam_obj),
            "score": float(adam_score),
            "temp_mae": float(adam_temp_mae),
            "disp_mae": float(adam_disp_mae),
            "temp_norm": float(adam_temp_norm),
            "disp_norm": float(adam_disp_norm),
            "checkpoint": os.path.basename(adam_ckpt_path),
            "selected": 0,
        }
    ]
    ...
    if not args.skip_lbfgs:
        ...
        if load_checkpoint_if_exists(model, lbfgs_ckpt_path):
            ...
            lbfgs_temp_mae, lbfgs_disp_mae = compute_reference_mae(model, reference_data)
            lbfgs_score, lbfgs_temp_norm, lbfgs_disp_norm = compute_stage_score(
                lbfgs_obj, lbfgs_temp_mae, lbfgs_disp_mae, reference_data, args
            )
            stage_rows.append(
                {
                    "stage": "lbfgs",
                    "objective": float(lbfgs_obj),
                    "score": float(lbfgs_score),
                    "temp_mae": float(lbfgs_temp_mae),
                    "disp_mae": float(lbfgs_disp_mae),
                    "temp_norm": float(lbfgs_temp_norm),
                    "disp_norm": float(lbfgs_disp_norm),
                    "checkpoint": os.path.basename(lbfgs_ckpt_path),
                    "selected": 0,
                }
            )
        else:
    ...
    if args.use_pso:
        ...
        if load_checkpoint_if_exists(model, pso_ckpt_path):
            ...
            pso_temp_mae, pso_disp_mae = compute_reference_mae(model, reference_data)
            pso_score, pso_temp_norm, pso_disp_norm = compute_stage_score(
                pso_obj, pso_temp_mae, pso_disp_mae, reference_data, args
            )
            stage_rows.append(
                {
                    "stage": "pso",
                    "objective": float(pso_obj),
                    "score": float(pso_score),
                    "temp_mae": float(pso_temp_mae),
                    "disp_mae": float(pso_disp_mae),
                    "temp_norm": float(pso_temp_norm),
                    "disp_norm": float(pso_disp_norm),
                    "checkpoint": os.path.basename(pso_ckpt_path),
                    "selected": 0,
                }
            )
        else:
    ...
    # Select best stage by composite score (objective + normalized reference MAE penalties)
    if not stage_rows:
        raise RuntimeError("No completed optimization stage found; cannot produce final checkpoint.")

    best_stage = min(stage_rows, key=lambda r: r["score"])
    ...
    meta = {
        ...
        "best_score": float(best_stage["score"]),
        "best_temp_mae": float(best_stage["temp_mae"]),
        "best_disp_mae": float(best_stage["disp_mae"]),
        "stage_score_w_temp": float(args.stage_score_w_temp),
        "stage_score_w_disp": float(args.stage_score_w_disp),
    ...
    print(
        ...
        f"| objective={best_stage['objective']:.6e} | score={best_stage['score']:.6e} "
        f"| final model: {final_ckpt}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=10000, help="Total Adam epochs")
    ...
    parser.add_argument("--phase1-epochs", type=int, default=1200, help="Data-focused Adam warmup epochs")
    parser.add_argument("--phase1-w-physics", type=float, default=1.0, help="Phase1 weight for physics loss")
    parser.add_argument("--phase1-w-ic", type=float, default=1e-5, help="Phase1 weight for IC loss")
    parser.add_argument("--phase1-w-bc", type=float, default=1e-18, help="Phase1 weight for BC loss")
    parser.add_argument("--phase1-w-data", type=float, default=5e-2, help="Phase1 weight for paper-data loss")
    parser.add_argument("--w-physics", type=float, default=5.0, help="Phase2 weight for physics loss")
    parser.add_argument("--w-ic", type=float, default=1e-4, help="Phase2 weight for initial condition loss")
    parser.add_argument("--w-bc", type=float, default=1e-18, help="Phase2 weight for boundary loss")
    parser.add_argument("--w-data", type=float, default=1e-2, help="Phase2 weight for paper-data loss")
    parser.add_argument(
        "--stage-score-w-temp",
        type=float,
        default=0.25,
        help="Composite-stage-score weight for normalized temperature MAE",
    )
    parser.add_argument(
        "--stage-score-w-disp",
        type=float,
        default=0.25,
        help="Composite-stage-score weight for normalized displacement MAE",
    )
