...
from Poisson_2D_Pymoo_Local_Common import LOCAL_N_GEN, LOCAL_POP_SIZE, run_local_nsga2_search
...
def main():
    save_dir = "./2D-Poisson-NSGA2-Pymoo"
    _, stop_logging = start_run_logging(save_dir)
    try:
        print(device)

        total_start = time.time()
        best_masks, search_meta = run_local_nsga2_search()
        model, loss_history, adam_time, lbfgs_time = train_final_model(best_masks)

        print_architecture(model, best_masks, "LOCAL PYMOO NSGA-II / NAS-PINN ARCHITECTURE")
        ...
        save_json(os.path.join(save_dir, "NSGA2_Pymoo_results.json"), results)
        save_json(os.path.join(save_dir, "NSGA2_Pymoo_evaluation.json"), evaluation, indent=4)
