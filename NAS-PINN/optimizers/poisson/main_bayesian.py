...
def decode_masks(params_dict):
    ...
    for i in range(LAYERS):
        raw = params_dict[f"m{i}"]
        clipped = int(np.clip(round(raw), 0, len(MASK_LEVELS) - 1))
        masks.append(clipped)
    return np.array(masks, dtype=int)


def run_bayesian_search(domain_mod, args):
    print(f"Starting Bayesian architecture search: domain={args.domain}")

    def objective(**kwargs):
        masks = decode_masks(kwargs)
        loss = proxy_evaluate(masks, domain_mod, proxy_epochs=args.proxy_epochs)
        print(f"BO eval masks={masks.tolist()} proxy_loss={loss:.4e}")
        return -loss

    pbounds = {f"m{i}": (0, len(MASK_LEVELS) - 1) for i in range(LAYERS)}
...
def run_single(args, domain_name, save_dir):
    ...
    best_masks = run_bayesian_search(domain_mod, args)
    print(f"Best mask selections: {best_masks}")
    model, loss_hist = train_with_best_masks(domain_mod, best_masks, args)

    plot_loss_history(
        loss_hist,
        save_path=os.path.join(save_dir, "poisson_bayesian_loss_curve.png"),
        title=f"Poisson Bayesian Loss ({domain_name})",
    )
    eval_model = FixedMaskModel(model, best_masks).to(device)
    rel_l2 = evaluate_and_plot(eval_model, domain_mod, save_dir=save_dir, method_label="Bayesian")

    run_time = time.perf_counter() - start
    save_run_metrics(save_dir, "bayesian", domain_name, args.seed, rel_l2, run_time)
...
def run_multi_domain(args):
    ...
    plt.title("Poisson Bayesian: Domain Comparison")
...
def parse_args():
    ...
    parser.add_argument("--proxy-epochs", type=int, default=PROXY_EPOCHS, help="proxy epochs per BO candidate")
    parser.add_argument("--bo-init-points", type=int, default=4, help="Bayesian init points")
    parser.add_argument("--bo-iters", type=int, default=12, help="Bayesian optimization iterations")
