...
def run_nsga3_search(train_points, args):
    ...
    ref_dirs = get_reference_directions("das-dennis", 2, n_partitions=max(8, args.search_pop // 2))
    algorithm = NSGA3(pop_size=args.search_pop, ref_dirs=ref_dirs)
...
def plot_results(model, save_dir):
    rel_l2 = plot_poisson_results(
        model,
        predict_on_grid,
        os.path.join(save_dir, "poisson_nsga3_results.png"),
        pred_title="Predicted (NSGA-III)",
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Poisson NAS-PINN with NSGA-III")
    ...
    parser.add_argument("--save-dir", type=str, default="results/poisson/nsga3", help="output directory")
    parser.add_argument("--checkpoint", type=str, default="poisson_checkpoint_last_nsga3.pth", help="checkpoint filename")

    parser.add_argument("--search-pop", type=int, default=20)
...
def main():
    ...
    if args.multi_seed:
        ...
        for seed_val in seeds:
            ...
            search_out = run_nsga3_search(train_points, args_local)
            ...
            plot_loss_curve(
                loss_history,
                os.path.join(run_dir, "poisson_nsga3_loss_curve.png"),
                title=f"Poisson NSGA-III Training Loss (seed={seed_val})",
            )
        ...
        plt.title("Poisson NSGA-III: Seed Comparison")
    ...
    search_out = run_nsga3_search(train_points, args)
    print("\nNSGA-III selected architecture")
    ...
    plot_loss_curve(
        loss_history,
        os.path.join(args.save_dir, "poisson_nsga3_loss_curve.png"),
        title="Poisson NSGA-III Training Loss",
    )
