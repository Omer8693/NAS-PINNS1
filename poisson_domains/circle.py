"""
NAS-PINN baseline for Poisson equation on unit circle domain x^2 + y^2 <= 1
...
# Sampling functions
# Rejection sampling for collocation points inside unit circle
def sample_collocation_points(n):
    x = []
    y = []
    while len(x) < n:
        x_try = np.random.uniform(-RADIUS, RADIUS, n)
        y_try = np.random.uniform(-RADIUS, RADIUS, n)
        mask = x_try**2 + y_try**2 <= RADIUS**2
        x.extend(x_try[mask])
        y.extend(y_try[mask])
    return np.array(x[:n]), np.array(y[:n])

# Uniform boundary points on circle (polar)
...
if __name__ == "__main__":
    ...
    save_dir = "results/poisson/circle/"
